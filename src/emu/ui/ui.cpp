...
UINT32 ui_manager::handler_load_save(running_machine &machine, render_container *container, UINT32 state)
{
	...
	if (state == LOADSAVE_SAVE)
	{
		machine.popmessage(_("Save to position %c"), file);
		machine.schedule_save(filename);
	}
	else
	{
		machine.popmessage(_("Load from position %c"), file);
	...
	for (screen_device *screen = scriter.first(); screen != NULL; screen = scriter.next())
		if (screen->screen_type() == SCREEN_TYPE_VECTOR)
		{
			...
			*tailptr = slider_alloc(machine, _("Vector Flicker"), 0, 0, 1000, 10, slider_flicker, NULL);
			tailptr = &(*tailptr)->next;
			*tailptr = slider_alloc(machine, _("Beam Width Minimum"), 1, 100, 1000, 1, slider_beam_width_min, NULL);
			tailptr = &(*tailptr)->next;
			*tailptr = slider_alloc(machine, _("Beam Width Maximum"), 1, 100, 1000, 1, slider_beam_width_max, NULL);
			tailptr = &(*tailptr)->next;
			*tailptr = slider_alloc(machine, _("Beam Intensity Weight"), -1000, 0, 1000, 10, slider_beam_intensity_weight, NULL);
			tailptr = &(*tailptr)->next;
			break;
		}
