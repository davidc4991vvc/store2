...
bool cheat_entry::activate()
{
	...
	if (is_oneshot())
	{
		execute_on_script();
		changed = true;
		manager().machine().popmessage(_("Activated %s"), m_description.c_str());
	}
...
void cheat_manager::set_enable(bool enable)
{
	...
	if (!m_disabled && !enable)
	{
		...
		machine().popmessage(_("Cheats Disabled"));
		m_disabled = true;
	}
	...
	else if (m_disabled && enable)
	{
		...
		machine().popmessage(_("Cheats Enabled"));
	}
}
